...
public class TencentApiController {
    @PostMapping("/DescribeRecordList")
    public Object getDescribeRecordList(@RequestParam String id ,String key,String domain){
        log.info("获取域名解析列表 id: {}, key: {}, domain:{}", id, key,domain);
        try{
    ...
    public Object getDeleteDomain(@RequestParam String id,String key,String domain){
        try{
            log.info("删除域名id: {}, key: {}, domain:{}", id, key,domain);
            // 实例化一个认证对象，入参需要传入腾讯云账户 SecretId 和 SecretKey，此处还需注意密钥对的保密
            // 代码泄露可能会导致 SecretId 和 SecretKey 泄露，并威胁账号下所有资源的安全性
            // 以下代码示例仅供参考，建议采用更安全的方式来使用密钥
            // 请参见：https://cloud.tencent.com/document/product/1278/85305
    ...
    public Object getModifyDomainStatus(@RequestParam String id ,String key ,String domain,String status){
        log.info("修改域名转态 id: {}, key: {}, domain:{}, status:{}", id, key,domain,status);
        try{
            // 实例化一个认证对象，入参需要传入腾讯云账户 SecretId 和 SecretKey，此处还需注意密钥对的保密
            // 代码泄露可能会导致 SecretId 和 SecretKey 泄露，并威胁账号下所有资源的安全性
            // 以下代码示例仅供参考，建议采用更安全的方式来使用密钥
    ...
    public Object getCreateDomain(@RequestParam String id ,String key ,String domain){
        log.info("添加域名 id: {}, key: {}, domain:{}", id, key,domain);
        try{
            // 实例化一个认证对象，入参需要传入腾讯云账户 SecretId 和 SecretKey，此处还需注意密钥对的保密
            // 代码泄露可能会导致 SecretId 和 SecretKey 泄露，并威胁账号下所有资源的安全性
            // 以下代码示例仅供参考，建议采用更安全的方式来使用密钥
